//Mogelijk zal deze file zich na een tijdje splitsen in een CPU_Functions.cpp en GPU_Functions.cpp file (idem voor de headers uiteraard).
	...
	Output=(float*)malloc(sizeof(float)*n*n);
		if (Output != NULL && data != NULL){
			...
			for(int row=0;row < n;row++){
				...
				for(int column=0; column < n; column++){
					float delta=0;
					if(n*(column)+row+n < n*n)
						delta=Output[n*(column)+row+n]-Output[row+n*column];
					if(std::abs(delta)>=TWOPI)
							deltaref=deltaref-delta;
					Output[row+n*column]=Output[row+n*column]+deltaref;
				}
			}
			...
			//	float deltaref=0;
			//	for (int row=0; row <= n; row++){
			//		float delta=Output[column+n*row+n]-Output[column+n*row];
			//		if(std::abs(delta)>=TWOPI){
			//			deltaref=deltaref-delta;
			//		}
			//		if(column+n*row+n < n*n)
			//			Output[column+n*row+n]=Output[column+n*row+n]+deltaref;
			//	}
			//}
		}
		else{
